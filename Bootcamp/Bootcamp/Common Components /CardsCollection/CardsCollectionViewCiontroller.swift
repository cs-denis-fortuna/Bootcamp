...
class CardsCollectionViewController: UIViewController {
    ...
    private var dataSource: CardsCollectionDataSource? {
        didSet {
            ...
            dataSource.didSelectCard = { [weak self] (cards, card, indexPath) in
                self?.showDetailForSelectedCard(cards, card: card, indexPath: indexPath)
            }
    ...
    // MARK: Navigation
    var coordinator: CardsCoordinatorProtocol?
...
extension CardsCollectionViewController {
    func showDetailForSelectedCard(_ cards: [Card], card: Card, indexPath: IndexPath) {
        coordinator?.didSelectCard(withCards: cards, selectedCard: card, andIndexPath: indexPath)
    }
}
